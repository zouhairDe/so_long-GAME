...
/*   Updated: 2024/02/07 19:08:48 by zouddach         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	wrong_button(int keycode)
{
	if (keycode != W_BUTTON && keycode != S_BUTTON && keycode != A_BUTTON
		&& keycode != D_BUTTON && keycode != ESCAPE_BUTTON && keycode
		!= UP_BUTTON && keycode != DOWN_BUTTON && keycode != LEFT_BUTTON
		&& keycode != RIGHT_BUTTON
		&& keycode != QUIT_BUTTON)
		return (1);
	return (0);
}
...
{
	if ((keycode == W_BUTTON || keycode == UP_BUTTON)
		&& data->map.map[data->player.pos.y - 1]
		[data->player.pos.x] != '1' && data->map.map[data->player.pos.y - 1]
		[data->player.pos.x] != 'E')
		return (1);
	else if ((keycode == S_BUTTON || keycode == DOWN_BUTTON)
		&& data->map.map[data->player.pos.y + 1]
		[data->player.pos.x] != '1' && data->map.map[data->player.pos.y + 1]
		[data->player.pos.x] != 'E')
		return (1);
	else if ((keycode == A_BUTTON || keycode == LEFT_BUTTON)
		&& data->map.map[data->player.pos.y]
		[data->player.pos.x - 1] != '1' && data->map.map[data->player.pos.y]
		[data->player.pos.x - 1] != 'E')
		return (1);
	else if ((keycode == D_BUTTON || keycode == RIGHT_BUTTON)
		&& data->map.map[data->player.pos.y]
		[data->player.pos.x + 1] != '1' && data->map.map[data->player.pos.y]
		[data->player.pos.x + 1] != 'E')
		return (1);
...
void	handle_move(t_data *data, int key)
{
	...
	if (key == W_BUTTON || key == UP_BUTTON)
		move_up(data);
	else if (key == S_BUTTON || key == DOWN_BUTTON)
		move_down(data);
	else if (key == A_BUTTON || key == LEFT_BUTTON)
		move_left(data);
	else if (key == D_BUTTON || key == RIGHT_BUTTON)
		move_right(data);
	simulate_game(data);
	ft_printf("%sPlayer has moved %d times%s\n",
		GREEN, data->player.moves, RESET);
	moves = ft_itoa(data->player.moves);
	mlx_string_put(data->mlx, data->mlx_win, 10, 7, 0x000, "Moves: ");
	mlx_string_put(data->mlx, data->mlx_win, 70, 7, 0x000, moves);
	free(moves);
}
...
int	key_hook(int keycode, t_data *data)
{
	if (wrong_button(keycode))
		ft_printf("%sError\nWrong BUTTON!!!%s\n", RED, RESET);
	else if (keycode == ESCAPE_BUTTON || keycode == QUIT_BUTTON)
		quite(data);
